...
class ViewTest(TestCase):
    ...
    def post_status_success(self):
        """Ensure that post method creates new object with it relations and status 201."""
        response = self.client.post('/api/v1/help/', json.dumps({
            "subject": "test subject", "message": "test message", "to":"test@,ail.com"}),
                                    content_type="application/json")
        self.assertEqual(response.status_code, 201)

    def test_post_status_400(self):
        """Ensure that post method returns status 400, when data is not passed."""
        response = self.client.post('/api/v1/help/', json.dumps({}), content_type="application/json")
