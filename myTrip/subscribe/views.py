...
class SubscribeView(View):
    ...
    def get(self, request, trip_id, subscribed_id, subscribe_id):
        """
        Handles GET request.
        ...
        Args:
            ...
            subscribe_id (int): id of Subscribe object
        Returns:
        ...
        subscribe = Subscribe.get_by_id(subscribe_id)
        if not subscribe:
            return HttpResponse(status=404)
        subscribe = subscribe.to_dict()
        return JsonResponse(subscribe, status=200)

    def post(self, request, subscribed_id, trip_id):
        """
        ...
        subscribe = Subscribe.filter(subscribed=subscribed, trip=trip)
        if subscribe:
            subscribe.delete()
